...
X_TRAIN, X_VAL, Y_TRAIN, Y_VAL, EMBEDDING_MATRIX = preprocessing(data_df=TRAIN_DF, is_training=True)


left_input = keras_input(shape=(utils.MAX_SEQ_LEN,), dtype='int32')
right_input = keras_input(shape=(utils.MAX_SEQ_LEN,), dtype='int32')

embedding_layer = keras_embedding(len(EMBEDDING_MATRIX), utils.EMBEDDING_DIM, weights=[EMBEDDING_MATRIX],
...
malstm_distance = keras_Lambda(function=lambda x: utils.exponent_neg_manhattan_distance(x[0], x[1]),
...
check_pointer = None
if utils.SAVE_MODEL:
    check_pointer = ModelCheckpoint(filepath=utils.DIR_WEIGHT_SAVING, verbose=2, save_best_only=True)

malstm.fit([X_TRAIN['left'], X_TRAIN['right']], Y_TRAIN,
                                batch_size=utils.BATCH_SIZE, nb_epoch=utils.N_EPOCH,
                                validation_data=([X_VAL['left'], X_VAL['right']], Y_VAL), callbacks=[check_pointer])
